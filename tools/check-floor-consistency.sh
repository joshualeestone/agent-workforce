#!/bin/bash
# The macOS floor lives in FOUR places that cannot read each other at
# runtime: tools/macos-floor (the builders read it), and install/setup.sh's
# MACOS_FLOOR_MAJOR/MINOR literals (the installer is a standalone file a
# user pipes from curl, so it cannot source a repo file). This check is what
# makes "the numbers cannot drift apart silently" a fact instead of a hope:
# it runs inside `yarn test`, so a drift fails the suite.
set -euo pipefail
