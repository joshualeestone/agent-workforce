...
/usr/bin/python3 "$TMP/merge.py" "$TMP/a/b/s1.json" || fail "fresh create exited nonzero"
/usr/bin/python3 - "$TMP/a/b/s1.json" <<'PY' || exit 1
import json, sys
...
/usr/bin/python3 "$TMP/merge.py" "$TMP/s2.json" || fail "merge exited nonzero"
/usr/bin/python3 - "$TMP/s2.json" <<'PY' || exit 1
import json, sys
d = json.load(open(sys.argv[1]))
...
if /usr/bin/python3 "$TMP/merge.py" "$TMP/s3.json" 2>/dev/null; then fail "unparseable did not refuse"; fi
...
/usr/bin/python3 "$TMP/merge.py" "$TMP/s4.json" || fail "already-true exited nonzero"
...
if /usr/bin/python3 "$TMP/merge.py" "$TMP/s5.json" 2>/dev/null; then fail "non-object did not refuse"; fi
...
/usr/bin/python3 "$TMP/merge.py" "$TMP/s6.json" || fail "tightened-file merge exited nonzero"
...
: > "$TMP/s7.json"
/usr/bin/python3 "$TMP/merge.py" "$TMP/s7.json" || fail "zero-byte file was refused"
/usr/bin/python3 - "$TMP/s7.json" <<'PY7' || exit 1
import json, sys
...
/usr/bin/python3 "$TMP/merge.py" "$TMP/s8.json" || fail "symlink merge exited nonzero"
[ -L "$TMP/s8.json" ] || fail "the merge severed the symlink"
/usr/bin/python3 - "$TMP/dotfiles/real-settings.json" <<'PY8' || exit 1
import json, sys
d = json.load(open(sys.argv[1]))
