...
test('the task routes: create over the wire, refusals write nothing, close and reopen, guard inherited', async () => {
  ...
  // An assigned create carries the block write's real outcome, in the TOLD
  // vocabulary, never a shape of its own.
  assert.ok(r2.told && typeof r2.told.state === 'string', 'an assigned create carried no told verdict');
  assert.ok(['told', 'could_not', 'not_tried'].includes(r2.told.state),
  ...
  assert.ok(rClose.told && ['told', 'could_not', 'not_tried'].includes(rClose.told.state),
  ...
  assert.ok(rReopen.told && ['told', 'could_not', 'not_tried'].includes(rReopen.told.state),
